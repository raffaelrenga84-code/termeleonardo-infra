...
Deno.test('la spunta «motivo storno» per persona, spenta per chi e nuovo', () => {
  ...
  assert(B.includes('storni: false, storno_con_motivo: false, cancella_tavolo: true, bloccato: false, nuovo: true'), 'un cameriere nuovo parte senza motivo e con il tavolo');
  assert(B.includes('data-k="cancella_tavolo"') && B.includes('>Cancella tavolo</th>'), 'la spunta «cancella tavolo» (la proprieta, 7 settembre 2026)');
