  ...
  if (azioniOspite.includes(azione)) {
    ...
    if (azione === 'ospite-menu') {
      const [cat, art, fas, pf] = await Promise.all([
        ...
        db.from('pos_articolo').select('id, categoria, nome, prezzo_cent, portata, esaurito, prezzo_libero, nomi, descrizioni, allergeni, orari, per_ospiti, vegano').eq('attivo', true),
