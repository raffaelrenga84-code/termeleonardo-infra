...
Deno.test('il menu per l ospite porta i nomi tradotti, le descrizioni e gli allergeni; le categorie il loro nome tradotto', () => {
  ...
  assert(o.includes("select('id, categoria, nome, prezzo_cent, portata, esaurito, prezzo_libero, nomi, descrizioni, allergeni, orari, per_ospiti, vegano, vegetariano').eq('attivo', true)"), 'articoli con nomi, descrizioni, allergeni');
});
...
Deno.test('«ospiti (QR)» sul singolo articolo: il palmare vede tutto, l ospite solo cio che il Bistrot ha davvero', () => {
  ...
  const o = S.slice(S.indexOf("azione === 'ospite-menu'"), S.indexOf("azione === 'ospite-stato'"));
  assert(o.includes("nomi, descrizioni, allergeni, orari, per_ospiti, vegano, vegetariano').eq('attivo', true)"), 'si legge la spunta');
