...
Deno.test('legge un corpo buono: stelle intere, temi noti senza doppioni, commento tagliato, tessera solo cifre', () => {
  ...
  assert(r.ok);
  assertEquals(r.valore, { lingua: 'de', stelle: 4, temi: ['camera', 'pulizia'], commento: 'Ottimo', tessera: '1000000014662' });
});
...
Deno.test('stelle fuori da 1-5 o non intere: errore; lingua ignota: italiano; commento vuoto: null; tessera con lettere: null', () => {
  ...
  assert(r.ok);
  assertEquals(r.valore, { lingua: 'it', stelle: 5, temi: [], commento: null, tessera: null });
...
Deno.test('l email: oggetto con le stelle e la camera o «anonima»; corpo con temi in italiano, commento, ora di Roma', () => {
  const e = emailOpinione({ lingua: 'en', stelle: 4, temi: ['cure', 'personale'], commento: 'Great staff', camera: '320', tesseraFallita: false, creatoIl: '2026-09-04T16:30:00Z', fonte: 'totem', prova: false });
  ...
  const a = emailOpinione({ lingua: 'it', stelle: 2, temi: [], commento: null, camera: null, tesseraFallita: true, creatoIl: '2026-09-04T16:30:00Z', fonte: 'totem', prova: true });
