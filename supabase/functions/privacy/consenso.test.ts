...
Deno.test('le due scelte, nell ordine del modulo; i testi hanno una versione e le stesse chiavi in quattro lingue', () => {
  /* due domande sole, come nel modulo che gli ospiti conoscono (4 set 2026) */
  assertEquals(SCELTE, ['conservazione', 'marketing']);
  /* la versione e la data, con un numero dopo se il testo cambia due volte in un giorno */
  assert(/^\d{4}-\d{2}-\d{2}(\.\d+)?$/.test(VERSIONE_TESTI), VERSIONE_TESTI);
...
Deno.test('leggiFirma: tre risposte booleane tutte presenti, firma PNG piccola, versione, fonte; altrimenti errore', () => {
  const buono = { id: 'a1', scelte: { conservazione: true, marketing: true }, firma: PNG, versione: VERSIONE_TESTI, fonte: 'totem', lingua: 'en' };
  const r = leggiFirma(buono);
  assert(r.ok);
  assertEquals(r.valore.scelte, { conservazione: true, marketing: true });
  assertEquals(r.valore.fonte, 'totem');
  assertEquals(leggiFirma({ ...buono, scelte: { conservazione: true } }).ok, false, 'manca una risposta');
  assertEquals(leggiFirma({ ...buono, scelte: { conservazione: 'si', marketing: true } }).ok, false, 'non booleana');
...
Deno.test('email alla reception in italiano con le tre scelte; all ospite nella sua lingua, con la revoca', () => {
  const c: PerEmail = { camera: '320', cognome: 'Rossi', nome: 'Mario', email: 'mario@example.com', lingua: 'en', scelte: { conservazione: true, marketing: true }, firmatoIl: '2026-09-04T16:30:00Z', fonte: 'totem', versione: VERSIONE_TESTI };
  const r = emailConsensoReception(c);
  assertEquals(r.oggetto, 'Privacy firmata: camera 320 · Rossi Mario');
  assert(r.testo.includes('Conservazione: sì') && r.testo.includes('Offerte: sì') && !r.testo.includes('Messaggi'), r.testo);
